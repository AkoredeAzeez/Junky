...
class Application(db.Model):
    ...
    supporting_documents = db.Column(db.String(256), nullable=True)  # Path to uploaded documents
    ...
    status = db.Column(db.String(20), default='pending')  # pending, verified, approved, funded, completed, rejected
    ...
    def __init__(self, patient_id, hospital_id, medical_condition, diagnosis, 
                 treatment_plan, estimated_cost, requested_amount, urgency_level, 
                 doctor_name, doctor_specialization, **kwargs):
    ...
    def funded_amount(self):
        ...
        funded = sum(donation.amount for donation in self.donations if donation.status == 'completed')
        return funded
...
class Donation(db.Model):
    ...
    general_fund = db.Column(db.Boolean, default=False)  # True if donation to general fund
    ...
    def __init__(self, transaction_id, donor_id, amount, application_id=None, 
                 general_fund=False, **kwargs):
...
class Transaction(db.Model):
    ...
    transaction_type = db.Column(db.String(20), nullable=False)  # donation, disbursement
    amount = db.Column(db.Float, nullable=False)
    ...
    def __init__(self, transaction_hash, application_id, transaction_type, 
                 amount, sender_address, recipient_address, **kwargs):
