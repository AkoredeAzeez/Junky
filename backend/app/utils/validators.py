...
class PatientSchema(Schema):
    ...
    condition_severity = fields.Integer(required=True, validate=validate.Range(min=1, max=10))
    medical_history = fields.String()
    current_medications = fields.String()
    insurance_status = fields.String(validate=validate.OneOf(
...
class ApplicationSchema(Schema):
    ...
    status = fields.String(validate=validate.OneOf(
        ['draft', 'submitted', 'under_review', 'approved', 'funded', 'completed', 'rejected']))
    
class ProposalSchema(Schema):
    """Schema for validating Proposal data"""
    title = fields.String(required=True, validate=validate.Length(min=10, max=100))
    description = fields.String(required=True, validate=validate.Length(min=50))
    applications = fields.List(fields.Integer(), required=True)  # List of application IDs
    ...
    status = fields.String(validate=validate.OneOf(
        ['draft', 'active', 'approved', 'rejected', 'executed']))
...
class VoteSchema(Schema):
    ...
    vote_type = fields.String(required=True, validate=validate.OneOf(['for', 'against']))
...
class TransactionSchema(Schema):
    ...
    transaction_type = fields.String(validate=validate.OneOf(
        ['donation', 'fund_disbursement', 'refund']))
